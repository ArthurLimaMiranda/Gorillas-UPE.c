...
//Cria uma janela para a entrada dos valores de angulo, velocidade, numero de rodadas e desafios
int EntradaNumerica(int modo, int tipo, int xTexto, int vMaximo){
	...
	while(!sair){
		...
		if((int)input== 10){
			...
			if((modo==3)&&((valor>=0)&&(valor<=vMaximo))){
				sair = true;
				wclear(janelaEntrada);
				wrefresh(janelaEntrada);
			}
			
			else if((valor>0)&&(valor<=vMaximo)){
				sair = true;
				wclear(janelaEntrada);
				wrefresh(janelaEntrada);
			}
			...
			if((int)input==127){
				...
				if((modo==3)&&((xDigitado-xTexto)==0) && ((input>=49)&&(input<=57))){
					mvwprintw(janelaEntrada,1,xDigitado+def,"%c", input);
					valor = (int)input-48;
					xDigitado++;
				}
				
				else if(((xDigitado-xTexto)==0) && ((input>=49)&&(input<=57))){
					mvwprintw(janelaEntrada,1,xDigitado+def,"%c", input);
					valor = (int)input-48;
					xDigitado++;
				}
...
//Cria uma janela para a entrada dos nomes dos usuario
//A funcao funciona mas crasha o jogo com frequencia, portanto ela esta desativada//
void EntradaNomes(int modo, char *entry){
	int yM, xM, xIni = 12, xDigitado=0, valorI, strTam=strlen(entry);
	bool sair = false;
	WINDOW * entradaWin;
	...
	for(valorI=0; valorI<strTam;valorI++){
		entry[valorI]=32;
	}
	...
	while(!sair){
		...
		if((int)input==10){
			...
			if((int)input==127){
				...
				if(xDigitado>0){
					xDigitado--;
					mvwprintw(entradaWin,1,(xDigitado+11)," ");
					entry[xDigitado]= 32;
					wrefresh(entradaWin);
				}
				//-------------------------------------//
			}
			else if((xDigitado>=0)&&(xDigitado<strTam)){
				//Atualiza o valor da variavel a medida que é inserido
				if((input>=33)&&(input<=126)){
					mvwprintw(entradaWin,1,(xDigitado+11),"%c", input);
					entry[xDigitado] = input;
					xDigitado++;		
				}					
				//--------------------------------------------------//
			}	
...
int main(int argc, char *argv[]) {
	...
	//Variaveis menu//
	int menuPos=0, menuOpt=0, tamanhoNome=10, desafios, desafiosTemp, tamanhoDesafios=4, rodadas, tamanhoRodadas=2, rodadaQtd=0, contRounds=0;
	int cont1=0, nLinhas=0, tamLinha=38;
	char nomeP1[] = {"Player1   "}, nomeP2[] = {"Player2   "}, linha[5][tamLinha], line;
	bool sairMenu=false, nP1=false, nP2=false, nR=false, nD=false, D1=false, D2=false, D3=false;
	char nomeFile[] = "ranking.dat";
	FILE * memoria;
	FILE * cache;
	
	//Checa se o arquivo existe. Caso nao exista, cria o arquivo
	...
	while(!exitar){	
		...
		while(!sairMenu){
			...
			while ((line = getc(memoria)) != EOF){	
				if(line!='\n'){
					linha[nLinhas][cont1]=line;
					cont1++;
				}
				else{
					while(cont1<tamLinha){
						linha[nLinhas][cont1]=32;
						cont1++;
					}
					nLinhas++;
					cont1=0;
			...
			if(menuPos==0){
				...
				if(control==10){
					switch(menuOpt){
						case 0:
							EntradaNomes(0, nomeP1);
							nP1=true;
							break;
						case 1:
							EntradaNomes(1, nomeP2);							
							nP2=true;
							break;
						case 2:
				...
				abasWin = newwin(40,42,8,xScr/2-10); 
				abasWinSub = subwin(abasWin, 40,42,8,xScr/2-10);
				wbkgd(abasWinSub, COLOR_PAIR(6));
				box(abasWin,0,0);
				...
				if(nLinhas>0){
					for(line=0;line<nLinhas;line++){
						for(cont1=0;cont1<tamLinha;cont1++){
							mvwprintw(abasWinSub,15+(2*line),3+cont1,"%c", linha[line][cont1]);
						}
					}
				}
		...
		//Checa os desafios
		desafiosTemp = desafios;
		for(cont1=0;cont1<tamanhoDesafios;cont1++){	
			if(desafiosTemp%10==1){
				D1=true;	
			}
			else if(desafiosTemp%10==2){
				D2=true;
			}
			else if(desafiosTemp%10==3){
				D3=true;
			}
		...
		while(!reset){
			...
			if(novoTiro){
					mvwprintw(stdscr, yScr-2, (macaco0Pos[1]+macaco1Pos[1])/2-1,"%d vs %d", M0score, M1score);
					mvwprintw(stdscr, 3, xScr/2-14,"Aperter qualquer tecla");				
				}
			
			//Checa numero de rodadas
			if(contRounds==(2*rodadas)){
				...
				memoria = fopen(nomeFile, "w");
				for (cont1=0;((nomeP1[cont1]!=32)||(cont1==tamanhoNome)); cont1++) {
		            fputc(nomeP1[cont1], memoria);
		        }
			
				fprintf(memoria, "(%dpts) vs ",M0score);
					
				for (cont1=0;((nomeP2[cont1]!=32)||(cont1==tamanhoNome)); cont1++) {
		            fputc(nomeP2[cont1], memoria);
		        }
				
				fprintf(memoria, "(%dpts)\n",M1score);
				
				if(nLinhas>0){
					for(line=0;line<4;line++){
						for(cont1=0;cont1<tamLinha;cont1++){
							fputc(linha[line][cont1], memoria);
						}
						fprintf(memoria, "\n");
					}
				}			
